	...
	: Super(ObjectInitializer.SetDefaultSubobjectClass<UShooterCharacterMovement>(ACharacter::CharacterMovementComponentName))
{
	...
	//setting shooter cmc
	ShooterCharacterMovementComponent = Cast<UShooterCharacterMovement>(GetCharacterMovement());
	ShooterCharacterMovementComponent->SetIsReplicated(true);
	...
	//get the pickup classes
	static ConstructorHelpers::FClassFinder<AShooterPickup> PickupGunOb(TEXT("/Game/Blueprints/Pickups/Pickup_AmmoGun"));
	PickupGunClass = PickupGunOb.Class;
	
	static ConstructorHelpers::FClassFinder<AShooterPickup> PickupLauncherOb(TEXT("/Game/Blueprints/Pickups/Pickup_AmmoLauncher"));
	PickupLauncherClass = PickupLauncherOb.Class;
...
float AShooterCharacter::TakeDamage(float Damage, struct FDamageEvent const& DamageEvent, class AController* EventInstigator, class AActor* DamageCauser)
{
	...
	if (ActualDamage > 0.f)
	{
		...
		TSubclassOf<UDamageType> WeaponDamageType = DamageEvent.DamageTypeClass;

		//Implementing Shrink
		if (WeaponDamageType.Get()->IsChildOf(UShrinkDamageType::StaticClass()))
		{
			//if Damage Event is Shrink	
			if (!ShooterCharacterMovementComponent->Is_ShrinkActive())
			{
				//Set wants to shrink
				ShooterCharacterMovementComponent->Set_ShrinkActive(true);
				return ActualDamage;
			}
		}
		else
		{
			//if already shrunk and shot again
			//nothing to do here
		}	

		//Implementing Freeze
		if (WeaponDamageType.Get()->IsChildOf(UFreezeDamageType::StaticClass()))
		{
			//if damage event is freeze
			//set wants to freeze
			if (!ShooterCharacterMovementComponent->Is_FreezeActive())
			{
				ShooterCharacterMovementComponent->Set_FreezeActive(true);
		...
		{
			//if already frozen and shot again
			//nothing to do here
		}
...
void AShooterCharacter::OnDeath(float KillingDamage, struct FDamageEvent const& DamageEvent, class APawn* PawnInstigator, class AActor* DamageCauser)
{
	...
	//Spawn the pickup
	AShooterWeapon* PlayerWeapon = GetWeapon();
	EWeaponType WeaponType = PlayerWeapon->GetWeaponType();
	UWorld* World = GetWorld();
	//If currently possessing a gun, spawn gun pickup
	...
	{		
		...
		AShooterPickup_Ammo* GunPickup = World->SpawnActor<AShooterPickup_Ammo>(PickupGunClass.Get(), GetActorLocation(), GetActorRotation(), SpawnInfo);
		if (GunPickup)
		{
			GunPickup->PickupWeapon = true;
			//Set pickup ammo
			GunPickup->PickupWeaponAmmo = PlayerWeapon->GetCurrentAmmo();
			GunPickup->SetupPickupWeapon();
		}
	}
	//If currently possesing a launcher, spawn launcher pickup
	...
	{		
		...
		SpawnInfo.SpawnCollisionHandlingOverride = ESpawnActorCollisionHandlingMethod::AlwaysSpawn;
		SpawnInfo.OverrideLevel = nullptr;
		AShooterPickup_Ammo* LauncherPickup = World->SpawnActor<AShooterPickup_Ammo>(PickupLauncherClass.Get(), GetActorLocation(), GetActorRotation(), SpawnInfo);
		if (LauncherPickup)
		{
			LauncherPickup->PickupWeapon = true;
			LauncherPickup->PickupWeaponAmmo = PlayerWeapon->GetCurrentAmmo();
			LauncherPickup->SetupPickupWeapon();
		}
	}
...
void AShooterCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	//My Inputs for CMC
	PlayerInputComponent->BindAction("TeleportPressed", IE_Pressed, this, &AShooterCharacter::OnTeleportPressed);
	PlayerInputComponent->BindAction("Jetpack", IE_Pressed, this, &AShooterCharacter::OnJetpackPressed);
	PlayerInputComponent->BindAction("Jetpack", IE_Released, this, &AShooterCharacter::OnJetpackReleased);
	PlayerInputComponent->BindAction("RewindPressed", IE_Pressed, this, &AShooterCharacter::OnRewindPressed);
	PlayerInputComponent->BindAction("ShrinkPressed", IE_Pressed, this, &AShooterCharacter::OnShrinkPressed);
	PlayerInputComponent->BindAction("FreezePressed", IE_Released, this, &AShooterCharacter::OnFreezePressed);
}
...
//My code
FCollisionQueryParams AShooterCharacter::GetIgnoreCharacterParams() const
{
...
void AShooterCharacter::OnTeleportPressed() {
	ShooterCharacterMovementComponent->TeleportPressed();
}

void AShooterCharacter::OnJetpackPressed() {
	ShooterCharacterMovementComponent->JetpackPressed();
}

void AShooterCharacter::OnJetpackReleased() {
	ShooterCharacterMovementComponent->JetpackReleased();
}

void AShooterCharacter::OnRewindPressed() {
	ShooterCharacterMovementComponent->RewindPressed();
}

void AShooterCharacter::OnShrinkPressed() {
	ShooterCharacterMovementComponent->ShrinkPressed();
}

void AShooterCharacter::OnFreezePressed() {
	ShooterCharacterMovementComponent->FreezePressed();
}

void AShooterCharacter::OnHit(UPrimitiveComponent* HitComp, AActor* OtherActor, UPrimitiveComponent* OtherComp,
	FVector NormalImpulse, const FHitResult& Hit)
{
	if (OtherActor)
	{
		AShooterCharacter* OtherShooterCharacter = Cast<AShooterCharacter>(OtherActor);
		if (OtherShooterCharacter)
		{
			//If other character is not shrunk and you are shrunk
			if (ShooterCharacterMovementComponent && OtherShooterCharacter->ShooterCharacterMovementComponent) {
				if (ShooterCharacterMovementComponent->Is_ShrinkActive() && !OtherShooterCharacter->ShooterCharacterMovementComponent->Is_ShrinkActive())
				{
					FDamageEvent StompDmg(UStompDamageType::StaticClass());
					TakeDamage(GetMaxHealth(), StompDmg, OtherShooterCharacter->GetController(), OtherActor);
				}
			}
		}
	}
