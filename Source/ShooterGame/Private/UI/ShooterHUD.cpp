...
void AShooterHUD::DrawJetpackFuel()
{
	...
	float ValueOffsetY = 225;
	float ValueBoxWidth = 215;

	// Jetpack draw text
	float ValuePosX = (Canvas->ClipX - Canvas->OrgX - (ValueBoxWidth + Offset) * ScaleUI);
	float ValuePosY = (Canvas->ClipY - Canvas->OrgY - (ValueOffsetY + Offset) * ScaleUI);

	Canvas->DrawIcon(KillsBg, ValuePosX, ValuePosY, ScaleUI);
	...
	FString Text;
	float value;
	Text = LOCTEXT("Jetpack", "JETPACK").ToString();
	value = MyPlayerState->GetJetpackFuel();
	Canvas->StrLen(BigFont, Text, SizeX, SizeY);
	...
	Canvas->DrawItem(TextItem, ValuePosX + KillsBg.UL * ScaleUI - ((ValueBoxWidth / 2 - 25) * ScaleUI) - (SizeX * TextScale * ScaleUI) / 2,
		ValuePosY + (KillsBg.VL* ScaleUI - SizeY * TextScale * ScaleUI) / 2);

	// Draw value of fuel left
	if (MyPlayerState) Text = FString::FromInt(value);
	else Text = FString("0");
	...
	TextItem.Text = FText::FromString(Text);
	if (value == 0) TextItem.SetColor(FColor(200, 0, 0, 255));
	TextItem.Scale = FVector2D(TextScale * ScaleUI, TextScale * ScaleUI);

	Canvas->DrawItem(TextItem, ValuePosX + Offset * ScaleUI + 1.5f * ScaleUI,
		ValuePosY + (KillsBg.VL * ScaleUI - SizeY * TextScale * ScaleUI) / 2);
}
...
void AShooterHUD::DrawRewindTime() {
	...
	const float RewindPosX = (Canvas->ClipX - HealthBarBg.UL * ScaleUI) / 2;
	const float RewindPosY = Canvas->ClipY - 2*(Offset + HealthBarBg.VL) * ScaleUI; //using 2 to leave space for health bar
	Canvas->DrawIcon(HealthBarBg, RewindPosX, RewindPosY, ScaleUI);
	const float RewindAmount = FMath::Min(1.0f, (MyPlayerState->GetRewindTime() / MyPlayerState->GetMaxRewindTime()));

	FCanvasTileItem TileItem(FVector2D(RewindPosX, RewindPosY), HealthBar.Texture->Resource,
		FVector2D(HealthBar.UL * RewindAmount  * ScaleUI, HealthBar.VL * ScaleUI), FLinearColor::Green);
	MakeUV(HealthBar, TileItem.UV0, TileItem.UV1, HealthBar.U, HealthBar.V, HealthBar.UL * RewindAmount, HealthBar.VL);
	TileItem.BlendMode = SE_BLEND_Additive;
	Canvas->DrawItem(TileItem);

	Canvas->DrawIcon(HealthIcon, RewindPosX + Offset * ScaleUI, RewindPosY + (HealthBar.VL - HealthIcon.VL) / 2.0f * ScaleUI, ScaleUI);
}

void AShooterHUD::DrawShrinkTime() {
	AShooterPlayerController* MyPC = Cast<AShooterPlayerController>(PlayerOwner);
	AShooterPlayerState* MyPlayerState = Cast<AShooterPlayerState>(MyPC->PlayerState);
	if (!MyPlayerState) {
		return;
	...
	Canvas->SetDrawColor(FColor::Orange);
	const float ShrinkPosX = (Canvas->ClipX - HealthBarBg.UL * ScaleUI) / 2;
	const float ShrinkPosY = Canvas->ClipY - 3 * (Offset + HealthBarBg.VL) * ScaleUI; //using 2 to leave space for health bar
	Canvas->DrawIcon(HealthBarBg, ShrinkPosX, ShrinkPosY, ScaleUI);
	const float ShrinkAmount = FMath::Min(1.0f, (MyPlayerState->GetShrinkTime() / MyPlayerState->GetMaxShrinkTime()));

	FCanvasTileItem TileItem(FVector2D(ShrinkPosX, ShrinkPosY), HealthBar.Texture->Resource,
		FVector2D(HealthBar.UL * ShrinkAmount  * ScaleUI, HealthBar.VL * ScaleUI), FLinearColor::Yellow);
	MakeUV(HealthBar, TileItem.UV0, TileItem.UV1, HealthBar.U, HealthBar.V, HealthBar.UL * ShrinkAmount, HealthBar.VL);
	TileItem.BlendMode = SE_BLEND_Additive;
	Canvas->DrawItem(TileItem);

	Canvas->DrawIcon(HealthIcon, ShrinkPosX + Offset * ScaleUI, ShrinkPosY + (HealthBar.VL - HealthIcon.VL) / 2.0f * ScaleUI, ScaleUI);
}
